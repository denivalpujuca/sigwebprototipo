...
export const MarcasPage: React.FC<MarcasPageProps> = ({ activeSection: externalActiveSection, onSectionChange: externalOnSectionChange, onLogout }) => {
  ...
  const [formData, setFormData] = useState({ nome: '', tipo: '', ativo: true });
    ...
    const content = `
      ...
      <html>
        ...
        <body>
          ...
          <p>Total de registros: ${filteredMarcas.length}</p>
          <table>
            <thead>
              <tr>
                <th>ID</th><th>Nome</th><th>Tipo</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              ${filteredMarcas.map(m => `
                <tr>
                  <td>${m.id}</td><td>${m.nome}</td><td>${m.tipo}</td><td>${m.ativo ? 'Ativo' : 'Inativo'}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
  ...
  const filteredMarcas = useMemo(() => {
    return marcas.filter(marca => {
      const matchesSearch = marca.nome.toLowerCase().includes(searchTerm.toLowerCase()) || marca.tipo.toLowerCase().includes(searchTerm.toLowerCase());
      const matchesStatus = filterStatus === 'all' || (filterStatus === 'ativos' && marca.ativo) || (filterStatus === 'inativos' && !marca.ativo);
  ...
  const handleSave = () => {
    ...
    if (editingMarca) {
      setMarcas(prev => prev.map(m => m.id === editingMarca.id ? { ...formData, id: editingMarca.id } : m));
    } else {
      const newId = Math.max(...marcas.map(m => m.id), 0) + 1;
      setMarcas(prev => [...prev, { ...formData, id: newId }]);
    }
    setIsModalOpen(false);
    setEditingMarca(null);
    setFormData({ nome: '', tipo: '', ativo: true });
  };

  const handleEdit = (marca: Marca) => {
    setEditingMarca(marca);
    setFormData({ nome: marca.nome, tipo: marca.tipo, ativo: marca.ativo });
    setIsModalOpen(true);
  };
  ...
  const content = (
    <>
      <nav className="flex items-center gap-2 text-xs text-[#555f70] mb-2 font-medium tracking-wide">
        <span className="text-[#191c1d]">Marcas</span>
      </nav>

      <div className="mb-8">
        <h1 className="text-[2.75rem] font-extrabold tracking-tight text-slate-900 mb-2" style={{ letterSpacing: '-0.02em' }}>Marcas</h1>
        <p className="text-[#555f70] text-sm">Gerenciamento de marcas de veículos e máquinas.</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-[0_20px_40px_rgba(25,28,29,0.06)] overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-300 border-b border-gray-300">
              {paginatedMarcas.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-[#555f70]">Nenhuma marca encontrada</td>
                </tr>
              ) : (
                paginatedMarcas.map(marca => (
                  <tr key={marca.id} className="hover:bg-[#f3f4f5]/50 transition-colors">
                    <td className="px-4 py-4 text-sm font-bold text-[#191c1d]">{marca.id}</td>
                    <td className="px-4 py-4 text-sm font-semibold text-[#191c1d]">{marca.nome}</td>
                    <td className="px-4 py-4 text-sm text-[#555f70]">{marca.tipo}</td>
                    <td className="px-4 py-4 text-center">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold ${marca.ativo ? 'bg-[#7ffc97] text-[#002109]' : 'bg-[#ffdad6] text-[#93000a]'}`}>
                        {marca.ativo ? 'Ativo' : 'Inativo'}
                      </span>
                    </td>
                    <td className="px-4 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => handleEdit(marca)} className="p-1.5 text-[#555f70] hover:text-[#006e2d] transition-colors">
                          <span className="material-symbols-outlined text-[20px]">edit</span>
                        </button>
                        <button onClick={() => handleToggle(marca.id)} className={`p-1.5 transition-colors ${marca.ativo ? 'text-[#555f70] hover:text-[#ba1a1a]' : 'text-[#006e2d] hover:opacity-70'}`}>
                          <span className="material-symbols-outlined text-[20px]">{marca.ativo ? 'block' : 'check_circle'}</span>
                        </button>
                      </div>
                    </td>
                  </tr>
        ...
        <div className="px-6 py-4 flex items-center justify-between bg-[#f3f4f5]/20">
          <span className="text-xs text-[#555f70] font-medium">Exibindo {paginatedMarcas.length} de {filteredMarcas.length} registros</span>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-white/85 backdrop-blur-[20px] rounded-xl shadow-[0_20px_40px_rgba(25,28,29,0.06)] w-full max-w-md mx-4 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#bccbb9]/10">
              <h2 className="text-lg font-bold text-[#191c1d]">{editingMarca ? 'Editar Marca' : 'Nova Marca'}</h2>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select value={formData.tipo} onChange={(e) => setFormData({ ...formData, tipo: e.target.value })} className="w-full px-4 py-2.5 bg-[#f8f9fa] border border-[#bccbb9]/20 rounded-md focus:ring-2 focus:ring-[#006e2d] text-sm">
