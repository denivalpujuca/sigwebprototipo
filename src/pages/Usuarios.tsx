...
export const UsuariosPage: React.FC<UsuariosPageProps> = () => {
  ...
  const handleSave = () => {
    if (editingUsuario) {
      setUsuarios(prev => prev.map(u => u.id === editingUsuario.id ? { ...formData, id: editingUsuario.id, ativo: editingUsuario.ativo, ultimoAcesso: editingUsuario.ultimoAcesso } : u));
    } else {
      const newId = Math.max(...usuarios.map(u => u.id), 0) + 1;
      setUsuarios(prev => [...prev, { ...formData, id: newId, ativo: true, ultimoAcesso: null }]);
    }
  ...
  const content = (
    <>
      ...
      <div className="mb-6">
        <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 mb-2" style={{ letterSpacing: '-0.02em' }}>Usuários</h1>
              ...
              {paginatedUsuarios.length === 0 ? (
                ...
                paginatedUsuarios.map(usuario => (
                  <tr key={usuario.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-4 text-sm text-slate-500">{formatarData(usuario.ultimoAcesso)}</td>
                    <td className="px-4 py-4 text-center">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold ${usuario.ativo ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                    ...
                    <td className="px-4 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button onClick={() => handleToggle(usuario.id)} className={`p-1.5 transition-colors ${usuario.ativo ? 'text-slate-500 hover:text-red-600' : 'text-emerald-600 hover:opacity-70'}`}>
